...
public final class TestHelper {
    ...
    public static UploadPictureRequestDto buildPictureRequestDto() throws IOException {
        UploadPictureRequestDto pictureRequestDto = new UploadPictureRequestDto();
        pictureRequestDto.setDescription("Picture description");
        pictureRequestDto.setPictureCategory(PictureCategory.machine);
        File file = new File("src/test/resources/Landscape-Color.jpg");
        FileInputStream input = new FileInputStream(file);
        MultipartFile multipartFile = new MockMultipartFile("file", file.getName(), "image/jpg", input.readAllBytes());
        pictureRequestDto.setFile(multipartFile);
        pictureRequestDto.setUserId(buildUserObject().getId());
        return pictureRequestDto;
    }
